...
// AUTO-GENERATED BY buildgql. DO NOT EDIT.
// Re-run \`npx buildgql generate\` after changing the schema.
`;
 ...
 * single root import and re-sorted rather than emitted as a second `from 'buildgql'` line,
 * so the default output stays byte-identical to what buildgql generated before this option
...
function importBlock(client: ClientKind): string {
  ...
  const fromRoot = module === 'buildgql' ? [...CORE_IMPORTS, ...names].sort() : [...CORE_IMPORTS].sort();
  const root = `import {\n${fromRoot.map((n) => `  ${n},`).join('\n')}\n  $,\n  v,\n} from 'buildgql';\n`;
  if (!module || module === 'buildgql') return root;
...
function reexportBlock(client: ClientKind): string {
  ...
  return `export { ${list.join(', ')} };\nexport type { Operation } from 'buildgql';\n`;
}
...
function assertNoScalarNameCollisions(ir: IRSchema, client: ClientKind): void {
  ...
  for (const name of declared) {
    if (reserved.has(name)) {
      throw new Error(
        `buildgql: a "scalars" entry maps to a TypeScript type named "${name}", which the generated ` +
          `module already binds. Give it a different "name" in your buildgql.config.*.`,
      );
    }
  }
}
...
export function emit(ir: IRSchema, client: ClientKind = 'buildgql'): string {
