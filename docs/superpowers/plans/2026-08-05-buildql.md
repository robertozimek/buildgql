...
**Goal:** Ship `buildql`, an npm library that turns a GraphQL introspection result into a fully type-safe TypeScript query-builder SDK, where result types and operation variables are both _inferred from the selection you write_ — no hand-written types, no manual `$name: String!` declarations.
...
1. **`$` must not be callable.** A mapped type over `string` (`{ [K in string]: VarMarker }`) does _not_ preserve the literal key — `$.name` is `VarMarker<string>`, not `VarMarker<'name'>`. So the variable name comes from the **argument key**, not the property access. And if `$` were also callable, `$.name`/`$.length`/`$.call` would resolve to `Function`'s own properties and silently drop the variable. Explicit naming therefore lives in a separate exported helper, `v('customName')`.
2. **Wrapper tuples need `const` type parameters.** `object('posts', ['!','l','!'], Post)` infers `W` as `("!"|"l")[]` — an array, not a tuple — unless declared `<const W extends Wrap>`. Without it `Apply<W, T>` silently falls through to `T | null` and _every field becomes nullable_.
...
```json
{
  ...
  "exports": {
    ...
    "./client": {
      ...
      "require": "./dist/client/client.cjs"
    },
    "./config": {
      "types": "./dist/cli/config.d.ts",
      "import": "./dist/cli/config.js",
      "require": "./dist/cli/config.cjs"
    }
  },
  "files": ["dist"],
  "scripts": {
...
Type tests are compile-only — a passing `tsc` _is_ the assertion.
...
`VarsOf<A, Spec>` is the pivot: `A` is the args object the user wrote (so it knows _which_ keys hold markers), `Spec` is the field's declared arg types from codegen (so it knows _what type_ each variable is). The variable's **name** comes from the arg key; the variable's **type** comes from the spec.
...
function applyDirective<N extends string, R, V, Name extends string>(
  ...
  const next = {
    ...(sel as object),
    directives: [...((sel as { directives?: DirectiveNode[] }).directives ?? []), directive],
  };
...
async function introspectSdl(source: string): Promise<IntrospectionResult> {
  ...
  return gql.executeSync({ schema, document: gql.parse(INTROSPECTION_QUERY) })
    .data as unknown as IntrospectionResult;
}
...
function inputTsType(ref: IRTypeRef, ir: IRSchema): string {
  const base =
    ref.kind === 'input' || ref.kind === 'enum' ? ref.name : (ir.scalars[ref.name] ?? UNKNOWN_SCALAR);
...
it('prints enum literals unquoted and strings quoted', () => {
  const E = {
    f: objectArgs(
      ...
      ['!'],
      User,
      args<{ status: 'ACTIVE' | 'BANNED'; name: string }>({ status: 'Status!', name: 'String!' }, ['status']),
    ),
  };
...
**Known limitation, document it in the README:** an enum nested inside an _input object_ literal still serialises as a quoted string, because the printer has no view of the input type graph. Pass such arguments as variables (`$.filter`) — JSON variable transport encodes enums as plain strings and is always correct. Add this to the README's Variables section.
...
it('executes generated operations against the real server', async () => {
  ...
  expect(posts).toEqual({
    posts: [{ id: 'p1', title: 'Hello', author: { id: 'u1', firstName: 'Ada', lastName: null } }],
  });
  ...
  const p = mod.query('Pet', ($: any, Q: any) => [
    Q.pet((P: any) => [
      mod.on('Dog', mod.Dog, (D: any) => [D.breed]),
      mod.on('Cat', mod.Cat, (C: any) => [C.lives]),
    ]),
  ]);
